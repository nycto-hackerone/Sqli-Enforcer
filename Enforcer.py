...
print'\033[38;5;1m███████║╚██████╔╝███████╗██║    ███████╗██║ ╚████║██║     ╚██████╔╝██║  ██║╚██████╗███████╗██║  ██║'
print'\033[38;5;1m╚══════╝ ╚══▀▀═╝ ╚══════╝╚═╝    ╚══════╝╚═╝  ╚═══╝╚═╝      ╚═════╝ ╚═╝  ╚═╝ ╚═════╝╚══════╝╚═╝  ╚═╝'
logo='  Coded By \033[38;5;1mNYCTO \033[38;5;46mPentesting \033[38;5;1mWeb-Security '
am='Sqli-Enforcer'
...
dork_dictionary = ['**********/fid17013034EFB2509745A39CD861F4FEA3E716FBE5.aspx?s=',
...
class get:
	def __init__(self,total_attempts=0,dork_attempts=0,vulns=0):
		...
		Space(7);print blue+'Use custom dorks'+green+'('+yellow+'cust'+green+')'+blue+' or dictionary'+green+'('+yellow+'dict'+green+')'+blue+' ?'
		answer=raw_input(blue + '✭'+red+'─ '+green)
		if answer =='cust' or answer == 'custom':
			...
			for i in aaa:
				dorks.append(i)
		elif answer=='dict' or answer == 'dictionary':
		...
		for dork in dorks:
			...
			for l in string:
				sys.stdout.write('\r')
				sys.stdout.write(blue+'['+green + 'x.x.x'+blue +']'+red +'H4CK3R'+blue +'['+green +'x.x.x'+blue + '] '+green+'%s'%(s.decode('utf8', errors='ignore')))
				s+='%s'%l
				sys.stdout.flush()
				sleep(.02)
			print
			...
			try:
				...
				links=list(set(re.findall('><cite class="_Rm">([A-Za-z0-9-=?./-:]+)',page)))
			for link in links:
				if 'Nycto' in link or 'nycto' in link:
...
class get:
	def __init__(self,total_attempts=0,dork_attempts=0,vulns=0):
		...
		for dork in dorks:
			...
			for l in string:
				sys.stdout.write('\r')
				sys.stdout.write(blue+'['+green + '-_-'+blue + '] '+green+'%s'%(s.decode('utf8', errors='ignore')))
				s+='%s'%l
				sys.stdout.flush()
				sleep(.02)
			print
